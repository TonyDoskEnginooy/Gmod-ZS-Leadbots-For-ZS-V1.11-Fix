...
local function IsThrowableProp(ent)
    ...
    if IsIgnoredPropModel(model) or IsBoardModel(model) then
        return false
    end
...
local function SelectThrowableProp(bot, target, props)
    if not HasEntries(props) then return nil end
...
local function TryThrowNearbyProp(bot, controller, foundEnts)
...
function LeadBot.StartCommand(bot, cmd)
    ...
    if IsValid(controller.Target) then
        ...
        TryThrowNearbyProp(bot, controller, foundEnts)
