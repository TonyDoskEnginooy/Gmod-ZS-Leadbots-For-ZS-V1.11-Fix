...
local function UpdateMovement(bot, controller, mv)
    ...
    local currentSegment = controller.cur_segment
    local currentGoal = controller.PosGen and segments[currentSegment] or nil
    ...
    if segments[currentSegment + 1]
        and Vector(bot:GetPos().x, bot:GetPos().y, 0):DistToSqr(Vector(currentGoal.pos.x, currentGoal.pos.y, 0)) < 100
    then
        controller.cur_segment = currentSegment + 1
        currentGoal = segments[controller.cur_segment]
    end

    local goalPosition = currentGoal.pos

    if bot:GetVelocity():Length2DSqr() <= 225 and not bot:IsFrozen() and not IsValid(controller.Target) then
    ...
    if controller.NextCenter < CurTime() then
        if currentGoal.area:GetAttributes() ~= NAV_MESH_JUMP and (bot:GetVelocity():Length2DSqr() <= 225 or IsValid(controller.Target)) then
    ...
    if controller.NextCenter > CurTime() then
        local canStrafe = currentGoal.area:GetAttributes() ~= NAV_MESH_JUMP
    ...
    if not bot:IsFrozen() and controller.NextJump ~= 0 and controller.NextJump < CurTime() then
        local isJumpGoal = currentGoal.type > 1 or currentGoal.area:GetAttributes() == NAV_MESH_JUMP
        if isJumpGoal then
            controller.NextJump = 0
        end
    end
    ...
    if currentGoal.area:GetAttributes() == NAV_MESH_CROUCH or IsValid(crouchTrace.Entity) then
        controller.NextDuck = CurTime() + 0.1
    end

    controller.goalPos = goalPosition

    local moveAngles = ((goalPosition + bot:GetCurrentViewOffset()) - bot:GetShootPos()):Angle()
